...
Après avoir modifié uniquement le dernier `COPY` de son Dockerfile, un développeur observe que le build affiche `--> Using cache` sur les huit premières étapes puis reconstruit les deux dernières. Le lendemain, il ajoute une variable `ENV` en **troisième** position et le build entier repart de zéro. Expliquez les deux comportements avec la même règle.
