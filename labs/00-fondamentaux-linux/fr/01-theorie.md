...
> **Podman** — Podman poussera cette logique jusqu'au bout : pas de service en arrière-plan, juste *votre* utilisateur (UID 1000) qui lance des processus. Tout ce labo — UID, signaux, `/proc`, ports non privilégiés — est la description exacte de ce que Podman a le droit de faire sans `sudo`. Docker, lui, s'appuie sur un service tournant en root ; la différence occupera le labo 01.
